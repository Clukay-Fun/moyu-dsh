window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/** Minimal observable controller exposed to other browser plugins. */
		var SettingsController = class {
			#nextId = 0;
			#request = null;
			#listeners = /* @__PURE__ */ new Set();
			/** Read the latest unconsumed open request. */
			getSnapshot() {
				return this.#request;
			}
			...
			/** Request that Settings open a registered section. */
			open(sectionId) {
				this.#request = {
					id: ++this.#nextId,
					sectionId
			...
			/** Clear one request after Settings has adopted it. */
			clear(id) {
		...
		* settings invalidation the mirror refreshes on (rc.2 lifecycle), plus the
		* root-scoped `settingsNavigation` bridge this kit adds on top.
		*/
		...
		*/
		function apply(ctx) {
			...
			ctx.effect(() => {
				const dispose = ctx.reflect.provide("settingsNavigation", settings);
				return () => {
					dispose();
				};

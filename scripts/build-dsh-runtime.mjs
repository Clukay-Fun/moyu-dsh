...
const PREINSTALLED_MODS = ['dsh-plugin-image', 'dsh-plugin-pdf']
...
const MOYU_PLUGINS = {
  ...
  // image / pdf 已迁为预装 Mod（C2-f，见 PREINSTALLED_MODS）。
...
async function buildProfileTemplate() {
  ...
  await writeFile(join(presetDir, 'preset.yml'), 'name: MOYU Chat\ndescription: MOYU 默认对话工作台\n')
  await writeFile(
